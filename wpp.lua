...
local function parsePeripheralUrl(peripheralUrl)
    ...
    if urlParts[1] == (currentProtocol .. ":") and urlParts[2] and urlParts[3] then
        return {
            clientId = tonumber(urlParts[2]),
            peripheralId = urlParts[3]
        }
    else
        return nil
    end
...
local function sendMessage(clientId, type, data)
    log("Sending message with type '" .. type .. "' to " .. currentProtocol .. "://" .. clientId .. " with data: " ..
...
local wrappedPeripheralApi = {
    ...
    call = function(clientId, peripheralName, methodName, ...)
        local args = ...
        log("Real call(" .. clientId .. ", " .. peripheralName .. ", " .. methodName .. ", " ..
                textutils.serialize(args) .. ")")
    ...
    if event[1] == "rednet_message" then
        if event[4] == currentProtocol then
            if event[3].version and event[3].version == CURRENT_VERSION then
                ...
                log("Recieved event from an unsupported WPP version. Only version '" .. CURRENT_VERSION ..
                        "' is supported.")
            end
        end
    end
...
function wireless.prefetchMethods(peripheralUrl, methods)
    ...
    if parsedPeripheralUrl == nil then
        ...
        sendMessage(parsedPeripheralUrl.clientId, "function", {
            func = "wppPrefetch",
            args = {parsedPeripheralUrl.peripheralId, methods}
        })
...
function remotePeripheral.call(peripheralUrl, method, ...)
    ...
    if parsedPeripheralUrl == nil then
        ...
        sendMessage(parsedPeripheralUrl.clientId, "function", {
            func = "call",
            args = {parsedPeripheralUrl.peripheralId, method, {...}}
        })
        local reply = recieveReply()
